...
MainLoop() {
    ...
    try {
        ...
        for _, id in ids {
            ...
            if (g_processed.Has(id)) {
                Log("Ignorando (memória): " id)
                continue
            }

            if (IsProcessedOnDisk(id)) {
                g_processed[id] := true
...
Api_GetNovosIds(token) {
    ...
    "function OutIds($x){" .
        ...
        "if($x -is [System.Array]){" .
            ...
            "elseif($first.PSObject.Properties.Match('id').Count -gt 0){ $x | ForEach-Object { Emit $_.id } }" .
        ...
        "} else {" .
            ...
            "elseif($x.PSObject.Properties.Match('id').Count -gt 0){ Emit $x.id }" .
