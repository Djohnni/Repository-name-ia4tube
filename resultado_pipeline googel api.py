...
# =========================================================
# IMPORTAÇÕES DO GOOGLE CLOUD VERTEX AI (IMAGEN 3)
# =========================================================
import vertexai
...
def render_via_google_api(output_path: Path, prompt: str, arquivos_referencia: list[Path]):
    image_files = []
    ...
    try:
        ...
        if image_files:
            log(f"🚀 Enviando {len(image_files)} imagens para OpenAI edit...")
            log(f"🧠 Modelo: {OPENAI_IMAGE_MODEL} | Size: {OPENAI_IMAGE_SIZE} | Quality: {OPENAI_IMAGE_QUALITY}")
            ...
            raw_path = output_path.with_suffix(f".raw.{OPENAI_OUTPUT_FORMAT}")
            ...
            log(f"✅ Resultado salvo pelo OpenAI edit em: {output_path}")
            return

        log("🚀 Inicializando Google Cloud Vertex AI (Imagen 3)...")
        ...
        log(f"🧠 Modelo: {MODEL_NAME} | Proporção: 9:16")

        imagens = model.generate_images(
            prompt=prompt,
            number_of_images=OPENAI_IMAGE_N,
            aspect_ratio="9:16"
        )

        temp_path = output_path.with_suffix(".jpeg")
        imagens[0].save(location=str(temp_path), include_generation_parameters=False)
        ...
        log(f"✅ Resultado salvo pelo Google Imagen em: {output_path}")
...
PROJECT_ID = "ia4tube-futebol"  # <<< COLOQUE SEU ID DO PROJETO AQUI
LOCATION = "us-central1"

OPENAI_URL = "https://api.openai.com/v1/responses"
OPENAI_MODEL = "gpt-4.1-mini"
# As variáveis abaixo de OPENAI_IMAGE foram mantidas por compatibilidade estrutural
...
# =========================================================
# NOVA FUNÇÃO DE GERAÇÃO: GOOGLE IMAGEN 3 API
# =========================================================
def render_via_google_api(output_path: Path, prompt: str, arquivos_referencia: list[Path]):
    log("🚀 Inicializando Google Cloud Vertex AI (Imagen 3)...")
    ...
    try:
        ...
        log(f"⚠️ Google falhou: {e}")
        log("🔁 Tentando fallback OpenAI...")
        ...
        try:
            ...
            if not image_files:
                raise ValueError("Nenhuma imagem de referência encontrada para fallback OpenAI.")
            ...
            log(f"✅ Resultado salvo pelo fallback OpenAI em: {output_path}")
...
# =========================================================
# NOVA FUNÇÃO DE GERAÇÃO: FUNDO (GOOGLE IMAGEN 3)
# =========================================================
def generate_background_image(prompt: str, output_path: Path):
    ...
    log("🚀 Gerando fundo com Google Cloud Vertex AI (Imagen 3)...")
    vertexai.init(project=PROJECT_ID, location=LOCATION)
    
    # O Google recomenda o modelo padrão para fundos ricos
    ...
    try:
        ...
        raise RuntimeError(f"Erro no Google Imagen: {e}")
...
def main():
    ...
    for jogador_id, jogador_fit in jogador_fits.items():
        ...
        jogador_text_render = jogador_textos.get(jogador_id, "") or str(bloco_cfg.get("text", ""))
        jogador_fits[jogador_id] = fit_block_text(
            jogador_text_render,
            FONT_REG,
            bloco_cfg,
            default_max_lines=int(bloco_cfg.get("max_lines", 1)),
            line_spacing=6
        )
    ...
    # A MÁGICA ACONTECE AQUI: A chamada agora é para a Vertex AI
    render_via_google_api(out_final_pedido, prompt_imagem, arquivos_chatgpt)
