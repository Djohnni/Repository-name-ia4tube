...
FONT_MAP = {
    "pesada impactante": str((BASE_DIR / "fonts" / "Anton-Regular.ttf") if (BASE_DIR / "fonts" / "Anton-Regular.ttf").exists() else Path(r"C:/Windows/Fonts/arialbd.ttf")),
    "condensada esportiva": str((BASE_DIR / "fonts" / "BebasNeue-Regular.ttf") if (BASE_DIR / "fonts" / "BebasNeue-Regular.ttf").exists() else Path(r"C:/Windows/Fonts/arialbd.ttf")),
    "moderna agressiva": str((BASE_DIR / "fonts" / "LeagueSpartan-Bold.ttf") if (BASE_DIR / "fonts" / "LeagueSpartan-Bold.ttf").exists() else Path(r"C:/Windows/Fonts/arialbd.ttf")),
    "tecnologica futurista": str((BASE_DIR / "fonts" / "Oswald-Bold.ttf") if (BASE_DIR / "fonts" / "Oswald-Bold.ttf").exists() else Path(r"C:/Windows/Fonts/arial.ttf")),
    "elegante forte": str((BASE_DIR / "fonts" / "Montserrat-Black.ttf") if (BASE_DIR / "fonts" / "Montserrat-Black.ttf").exists() else Path(r"C:/Windows/Fonts/arial.ttf"))
}
...
def drawtext_block(text: str, font_path: str, font_size: int, color_hex: str, x_expr: str, y_expr: str, line_spacing: int = 6, alpha: str = "") -> str:
    ...
    stroke_hex = rgb_to_hex(choose_stroke_color(hex_to_rgb(color_hex)))
    border_w = max(4, int(round(font_size * 0.10)))
...
def main():
    ...
    FONT_BOLD = r"C:/Users/USER/Desktop/resultado/IA4tube/fonts/Anton-Regular.ttf"
    FONT_REG = r"C:/Users/USER/Desktop/resultado/IA4tube/fonts/Montserrat-Black.ttf"
    ...
    bg_prompt = (
        f"Fundo de confronto cinematográfico em estádio de futebol à noite, vertical 9:16, "
        f"lado esquerdo usando EXATAMENTE a cor {rgb_to_hex(bg_local_1)}, "
        f"lado direito usando EXATAMENTE a cor {rgb_to_hex(bg_local_2)}, "
        f"sem mudar a tonalidade original dos escudos, sem dourado genérico, sem azul genérico, "
        f"com fumaça e luzes nessas mesmas cores reais, energia no centro, grama visível, "
        f"névoa realista, textura rica, profundidade de campo, sem texto, sem escudos, sem pessoas."
    )
