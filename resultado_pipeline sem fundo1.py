...
def get_palette_from_openai(crest_path: Path, foto_jogo: Path, team_name: str = "") -> dict:
    ...
    color_map_raw = parsed.get("color_map", {})
    color_map = {}

    if isinstance(color_map_raw, dict):
        for nome_cor, destino_hex in color_map_raw.items():
            nome_cor = str(nome_cor).strip().lower()
            destino_hex = str(destino_hex).strip()
            try:
                hex_to_rgb(destino_hex)
                color_map[nome_cor] = destino_hex
            except Exception:
                pass
...
# =========================================================
# RECOLORIZACAO POR COLOR_MAP
# =========================================================
RANGES_HSV_CORES = {
    "vermelho": [
        (np.array([0, 40, 25]), np.array([10, 255, 255])),
        (np.array([170, 40, 25]), np.array([179, 255, 255])),
    ],
    "laranja": [
        (np.array([11, 40, 25]), np.array([20, 255, 255])),
    ],
    "amarelo": [
        (np.array([21, 40, 25]), np.array([35, 255, 255])),
    ],
    "verde": [
        (np.array([36, 35, 25]), np.array([85, 255, 255])),
    ],
    "ciano": [
        (np.array([86, 35, 25]), np.array([100, 255, 255])),
    ],
    "azul": [
        (np.array([101, 35, 25]), np.array([130, 255, 255])),
    ],
    "roxo": [
        (np.array([131, 35, 25]), np.array([169, 255, 255])),
    ],
    "preto": [
        (np.array([0, 0, 0]), np.array([179, 60, 70])),
    ],
    "grafite": [
        (np.array([0, 0, 25]), np.array([179, 70, 120])),
    ],
    "branco": [
        (np.array([0, 0, 180]), np.array([179, 50, 255])),
    ],
    "cinza": [
        (np.array([0, 0, 70]), np.array([179, 50, 190])),
    ],
}

def hex_para_hsv(hex_str: str):
    hex_str = hex_str.strip().lstrip("#")
    if len(hex_str) != 6:
        raise ValueError(f"HEX inválido para HSV: {hex_str}")
    r, g, b = tuple(int(hex_str[i:i+2], 16) for i in (0, 2, 4))
    pixel_bgr = np.uint8([[[b, g, r]]])
    hsv = cv2.cvtColor(pixel_bgr, cv2.COLOR_BGR2HSV)[0][0]
    return hsv

def _mascara_nome_cor(hsv_img, nome_cor: str):
    nome_cor = str(nome_cor).strip().lower()
    ranges = RANGES_HSV_CORES.get(nome_cor, [])
    if not ranges:
        return None
...
def aplicar_color_map(src_path: Path, dst_path: Path, color_map: dict, fallback_hex: str, min_alpha_keep: int = 20):
    img = cv2.imread(str(src_path), cv2.IMREAD_UNCHANGED)
    if img is None:
        raise FileNotFoundError(f"Não foi possível abrir template: {src_path}")
    ...
    hsv_base_u8 = cv2.cvtColor(bgr, cv2.COLOR_BGR2HSV)

    if alpha is not None:
        alpha_mask = alpha >= min_alpha_keep
    else:
        alpha_mask = np.ones((hsv.shape[0], hsv.shape[1]), dtype=bool)

    alguma_aplicada = False

    for nome_cor, destino_hex in (color_map or {}).items():
        mask = _mascara_nome_cor(hsv_base_u8, nome_cor)
        if mask is None:
            continue

        mask_bool = (mask > 0) & alpha_mask
        if not np.any(mask_bool):
            continue

        destino_hsv = hex_para_hsv(destino_hex)
        target_h = float(destino_hsv[0])
        target_s = float(destino_hsv[1])

        sat_original = hsv[:, :, 1].copy()
        sat_nova = (sat_original * 0.25) + (target_s * 0.75)
        sat_nova = np.where(sat_nova < 35, max(35.0, target_s * 0.55), sat_nova)
        sat_nova = np.clip(sat_nova, 0, 255)

        hsv[:, :, 0] = np.where(mask_bool, target_h, hsv[:, :, 0])
        hsv[:, :, 1] = np.where(mask_bool, sat_nova, hsv[:, :, 1])

        alguma_aplicada = True

    if not alguma_aplicada:
        destino_hsv = hex_para_hsv(fallback_hex)
        target_h = float(destino_hsv[0])
        target_s = float(destino_hsv[1])

        sat_original = hsv[:, :, 1].copy()
        sat_nova = (sat_original * 0.35) + (target_s * 0.65)
        sat_nova = np.where(sat_nova < 35, max(35.0, target_s * 0.55), sat_nova)
        sat_nova = np.clip(sat_nova, 0, 255)

        hsv[:, :, 0] = np.where(alpha_mask, target_h, hsv[:, :, 0])
        hsv[:, :, 1] = np.where(alpha_mask, sat_nova, hsv[:, :, 1])

    bgr_final = cv2.cvtColor(hsv.astype(np.uint8), cv2.COLOR_HSV2BGR)

    if alpha is not None:
        out = np.dstack([bgr_final, alpha])
    else:
        out = bgr_final

    ok = cv2.imwrite(str(dst_path), out)
    if not ok:
        raise RuntimeError(f"Falha ao salvar template recolorido: {dst_path}")
...
def main():
    ...
    try:
        ...
        color_map = palette.get("color_map", {})
    ...
    log(f"Color map: {json.dumps(color_map, ensure_ascii=False)}")
    ...
    if moldura_src:
        aplicar_color_map(
            ...
            moldura_t,
            color_map=color_map,
            fallback_hex=rgb_to_hex(cor_secundaria)
        )
    if topo_src:
        aplicar_color_map(
            topo_src,
    ...
    if faixa1_src:
        aplicar_color_map(
            ...
            faixa1_t,
            color_map=color_map,
            fallback_hex=rgb_to_hex(cor_primaria)
        )
    if faixa2_src:
        aplicar_color_map(
            faixa2_src,
    ...
    filter_parts.append(
        f"[canvas0][foto_fit]overlay="
        f"{photo_box_x}+({photo_box_w}-w)/2:"
        f"{photo_box_y}+({photo_box_h}-h)/2"
        f"[base0]"
    )
