...
def choose_text_color(bg_rgb):
    return (255, 255, 255) if luminance(bg_rgb) < 150 else (0, 0, 0)
...
# =========================================================
# MOTOR TXT (igual ao teste)
# =========================================================
CORES_UNIVERSAIS = {
    'vermelho': {'baixo': [0, 30, 30], 'alto': [10, 255, 255]},
    'amarelo': {'baixo': [15, 30, 30], 'alto': [40, 255, 255]},
    'verde': {'baixo': [40, 30, 30], 'alto': [85, 255, 255]},
    'ciano': {'baixo': [85, 30, 30], 'alto': [105, 255, 255]},
    'azul': {'baixo': [100, 30, 30], 'alto': [135, 255, 255]},
    'roxo': {'baixo': [135, 30, 30], 'alto': [160, 255, 255]},
...
def criar_mascara(hsv, cor_nome):
    if cor_nome == 'vermelho':
        baixo1 = np.array([0, 30, 30])
        alto1  = np.array([10, 255, 255])
        baixo2 = np.array([170, 30, 30])
...
def detectar_cores_na_imagem(img_path: Path, cobertura_minima: float = 0.01) -> list[str]:
    """
    Detecta quais cores nomeadas existem na imagem.
    Ignora preto e branco.
    """
...
def gerar_txt_automatico_por_cor_detectada(src_path: Path, txt_path: Path, cor_destino_hex: str):
    """
    Gera automaticamente um TXT no formato:
    azul = #AABBCC
    vermelho = #AABBCC
    """
    if txt_path.exists():
...
def aplicar_regras_txt(src_path: Path, dst_path: Path, txt_path: Path):
    ...
    for cor_origem, cor_destino in regras.items():
        ...
        if config_destino['is_color']:
            ...
            if 's' in config_destino:
                hsv_novo[:, :, 1] = np.where(mascara > 0, config_destino['s'], hsv_novo[:, :, 1])

            if 'v' in config_destino:
                hsv_novo[:, :, 2] = np.where(mascara > 0, config_destino['v'], hsv_novo[:, :, 2])
        else:
            hsv_novo[:, :, 1] = np.where(mascara > 0, config_destino['s'], hsv_novo[:, :, 1])
...
def get_palette_from_openai(crest_path: Path, foto_jogo: Path, team_name: str = "") -> dict:
    ...
    faixa_template = find_existing(
        TEMPLATE_DIR,
        ["faixa_secundaria.png", "faixa_secundaria.jpg", "faixa_secundaria.jpeg"]
    )

    if not faixa_template:
        raise FileNotFoundError("faixa_secundaria não encontrada")

    image_data_url_template = image_to_data_url(faixa_template)
                ...
                "content": [
                    {
                        ...
                        "text": (
                            ...
                            + "\n\nMISSÃO ESPECÍFICA:"
                            + "\n- Analise a faixa_secundaria.png."
                            + "\n- Identifique as cores reais existentes nela."
                            + "\n- Cruze isso com a foto do jogo e com o escudo."
                            + "\n- Gere o campo txt_faixa_secundaria com as trocas corretas."
                            + "\n- O txt_faixa_secundaria será aplicado diretamente sobre a imagem original faixa_secundaria.png."
...
def main():
    ...
    try:
        ...
        if luminance(cor_secundaria) < 90:
            cor_secundaria = (255, 209, 0)
    ...
    faixa2_src = optional_template_file(
        TEMPLATE_DIR,
        ["faixa_secundaria.png", "faixa_secundaria.jpg", "faixa_secundaria.jpeg"]
    )
    ...
    if draw_chain:
        filter_parts.append(f"[final_base]{draw_chain}[out]")
    else:
        filter_parts.append(f"[final_base]copy[out]")
